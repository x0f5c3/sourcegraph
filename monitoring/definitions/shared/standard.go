	...
	return func(options ObservableConstructorOptions) sharedObservable {
		return func(containerName string, owner monitoring.ObservableOwner) Observable {
			...
			return Observable{
				...
				Description: fmt.Sprintf("%s%s error rate", options.MetricDescriptionRoot, legend),
				Query: fmt.Sprintf(`sum%[1]s(src_%[2]s_errors_total{%[3]s}) / (sum%[1]s(src_%[2]s_total{%[3]s}) + sum%[1]s(src_%[2]s_errors_total{%[3]s})) * 100`,
