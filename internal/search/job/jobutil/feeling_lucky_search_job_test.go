...
func Test_patternsAsFilters(t *testing.T) {
	...
	rules := []rule{
		{
			description: "patterns to filters",
	...
	t.Run("URL tree", func(t *testing.T) {
		autogold.Equal(t, autogold.Raw(test(`https://github.com/sourcegraph/sourcegraph/tree/main/lib`, rules)))
	})

	t.Run("looks like a path", func(t *testing.T) {
		autogold.Equal(t, autogold.Raw(test(`lib/output/output_windows.go`, rules)))
	})
}
