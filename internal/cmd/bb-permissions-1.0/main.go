...
func main() {
	...
	logFilepath := flag.String("logfile", "permissions.log", "path to a log file")
	...
	if len(*scratchDir) == 0 {
		d, err := os.MkdirTemp("", "bb-permissions")
		if err != nil {
	...
	resp, err := client.DefaultApi.GetUsers(nil)
	users, err := bbv1.GetUsersResponse(resp)

	for i, user := range users {
		//userDebug, err := fmt.Printf("%+v\n", user)
		//if err != nil {
		//	log15.Error("can't print user", "i", i)
		//}
		log15.Info("log user in users:", "i", i, "user name", user.Name)
		//log15.Debug("debug log user in users:", "i", i, "user", userDebug)

		userMarshal, _ := json.Marshal(user)
		log15.Info(string(userMarshal))
	}

	testUser := "user01"
	resp, err = client.DefaultApi.GetUser(testUser)
	//
	//if err != nil {
	...
	//}
	var user bbv1.User
	err = mapstructure.Decode(resp.Values, &user)

	//user, err := bbv1.GetUsersResponse(resp)
	log15.Info("log user in users:", "user name", user.Name)
	//log15.Debug("debug log user in users:", "i", i, "user", userDebug)

	userDebug, err := fmt.Printf("%+v\n", user)
	if err != nil {
		log15.Error("can't print user")
	}
	log15.Debug("debug log user:", "user", userDebug)
	userMarshal, _ := json.Marshal(user)
	log15.Debug("debug log user json.Marshal:", "user", string(userMarshal))

	resp, err = client.DefaultApi.GetRepository("ELATED-JANG-356", "zoekt")
	repo, err := bbv1.GetRepositoryResponse(resp)
	log15.Debug("debug log repo:", "repo", repo.Slug)
	repoMarshal, _ := json.Marshal(repo)
	log15.Debug("debug log repoMarshal json.Marshal:", "repo", string(repoMarshal))

	//resp, err = client.DefaultApi.GetUsersWithAnyPermission_23(testUser)
	//var user bbv1.UserPermission
