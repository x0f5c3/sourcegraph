...
func TestBranchEventSummary(t *testing.T) {
	t.Run("unlocked", func(t *testing.T) {
		...
		want := autogold.Want("branch-event.unlocked", ":white_check_mark: Pipeline healthy - `main` unlocked!")
		want.Equal(t, got)
	})
...
func TestWeeklySummary(t *testing.T) {
	...
	want := autogold.Want("summary", `:bar_chart: Welcome to the weekly CI report for period *2006-01-02* to *2006-01-03*!
